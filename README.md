...
2. Меняем статус одного из созданных:
   ```shell
   curl -X PATCH --location "http://localhost:8081/api/v1/incidents/1/" \
    -H "Content-Type: application/json" \
    -d '{
          "status": "acknowledged"
        }'
...
4. Пробуем несуществующий инцидент:
   ```shell
   curl -X PATCH --location "http://localhost:8081/api/v1/incidents/0/" \
    -H "Content-Type: application/json" \
    -d '{
          "status": "acknowledged"
        }'
...
инциденту. Также я добавил карту переходов статусов и минимальную логику контроля правильности переходов.

Чтобы сократить время, я использовал заготовки логирования, конфигурации и работы с базой из своих проектов. Поэтому
где-то код может показаться избыточным.

В реальной системе я бы еще добавил:

- Аутентификацию, хотя бы на базе токенов.
- Rate limiting.
- Исключение дублирование запросов от клиента (например, из-за ошибок сети). Для этого
  можно требовать уникальный идентификатор запроса от клиента и проверять по нему, был ли такой запрос уже обработан
  успешно.